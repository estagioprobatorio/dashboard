...
async function runImport() {
  ...
  // Reduzido para 100 registros por lote para evitar sobrecarga no Supabase Free Tier
  const batchSize = 100;
  let totalInserted = 0;
  ...
  for (let i = 0; i < mappedRecords.length; i += batchSize) {
    ...
    if (error) {
      console.error(`Erro ao inserir lote iniciando em ${i}:`, error);
    } else {
      totalInserted += batch.length;
      console.log(`Lote inserido com sucesso. Progresso: ${totalInserted}/${mappedRecords.length}`);
    }

    // Intervalo de 1.5 segundos entre lotes para o servidor respirar
    await sleep(1500);
  }

  console.log("Processo de importação finalizado com sucesso!");
}
